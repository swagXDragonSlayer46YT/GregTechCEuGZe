...
public class BlastFurnaceRecipeLogic extends AbstractRecipeLogic {
    ...
    protected void trySearchNewRecipe() {
        MetaTileEntityBlastFurnace blastfurnace = (MetaTileEntityBlastFurnace) metaTileEntity;
        if (ConfigHolder.machines.enableMaintenance && blastfurnace.hasMaintenanceMechanics() &&
                blastfurnace.getNumMaintenanceProblems() > 5) {
            return;
        }

        // can optimize with an override of checkPreviousRecipe() and a check here

        IMultipleTankHandler importFluids = blastfurnace.getImportFluids();
        ...
        for (IFluidTank fluidTank : importFluids.getFluidTanks()) {
            ...
            if (dieselRecipe != null &&
                ...
                setMaxProgress(adjustBurnTimeForThrottle(Math.max(1, (int)(
            ...
            if (denseFuelRecipe != null &&
                ...
                setMaxProgress(adjustBurnTimeForThrottle(
                        Math.max(1, (int)(1.5 * (Math.abs(denseFuelRecipe.getEUt()) *
        ...
        if (!didStartRecipe) {
            IItemHandlerModifiable importItems = blastfurnace.getImportItems();
            for (int i = 0; i < importItems.getSlots(); i++) {
                ...
                if (fuelBurnTime / 80 > 0) { // try to ensure this fuel can burn for at least 1 tick
                    ...
                    setMaxProgress(excessProgress +
                            adjustBurnTimeForThrottle(150 * (fuelBurnTime / 80) / 100));
                    stack.shrink(1);
                    didStartRecipe = true;
                    break;
                }
        ...
        if (didStartRecipe) {
            this.progressTime = 1;
            this.recipeEUt = adjustEUtForThrottle(1800);
