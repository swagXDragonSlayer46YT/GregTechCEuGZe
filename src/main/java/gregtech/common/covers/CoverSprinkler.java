...
import gregtech.api.unification.material.properties.FertilizerProperty;
import gregtech.api.unification.material.properties.PropertyKey;
import gregtech.client.particle.GTSprinkle;
...
import static gregtech.api.capability.GregtechDataCodes.UPDATE_SPRINKLER_DATA;
...
public class CoverSprinkler extends CoverBase implements CoverWithUI, ITickable {
    ...
    public void renderCover(CCRenderState renderState, Matrix4 translation, IVertexOperation[] pipeline, Cuboid6 plateBox, BlockRenderLayer blockRenderLayer) {
        if (wasWorking && showsSprinkles) {
            ...
            if (Math.random() < chance) {
                Minecraft.getMinecraft().effectRenderer.addEffect(new GTSprinkle(this.getCoverableView().getWorld(), this.getCoverableView().getPos().getX() + 0.5, this.getCoverableView().getPos().getY() - 0.1, this.getCoverableView().getPos().getZ() + 0.5, operationPosition.getX() + 0.5, operationPosition.getY() + 1, operationPosition.getZ() + 0.5, sprinkleColor));
            }
            updateOperationPosition();
        }
        Textures.SPRINKLER_OVERLAY.renderSided(this.getAttachedSide(), plateBox, renderState, pipeline, translation);
    }
    ...
    public void update() {
        ...
        if (isWorkingNow) {
            ...
            if (mat != null) {
                ...
                FertilizerProperty property = mat.getProperty(PropertyKey.FERTILIZER);
                if (property != null) {
    ...
    public EnumActionResult onSoftMalletClick(EntityPlayer playerIn, EnumHand hand, CuboidRayTraceResult hitResult) {
        ...
        if (!this.isRemote()) {
            playerIn.sendMessage(new TextComponentTranslation(showsSprinkles ? "gregtech.sprinkler.particles.on" : "gregtech.sprinkler.particles.off"));
        }
