...
public class BlockGTBush extends BlockGTCrop {
    ...
    protected BlockGTBush(String name) {
        super(name, 2);
    ...
    public static BlockGTBush create(String name) {
        AGE_TEMP = PropertyInteger.create("age", 0, 2);
        return new BlockGTBush(name);
    }
    ...
    public IBlockState withEfficiency(IBlockState state, int efficiency) {
        if (efficiency > 4) {
            efficiency = 4;
            GTLog.logger.warn("Somehow, you managed to get your berry's efficiency higher than 4, which is really cool (or the result of a hacked mod/bug), but it's currently not available in GTFO. Please report this to the mod author, along with a screenshot of how great your berry setup is.");
        }
    ...
    public BlockGTBush setThorny(boolean thorny) {
        isThorny = thorny;
        return this;
    }
    ...
    public void neighborChanged(IBlockState state, World worldIn, BlockPos pos, Block blockIn, BlockPos fromPos) {
        if (!(worldIn.getBlockState(fromPos).getBlock() instanceof BlockGTBush)) {
