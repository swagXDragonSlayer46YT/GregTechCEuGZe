...
public class CyanidePoisoningPotion extends GTPotion {
    ...
    public void performEffect(EntityLivingBase entity, int amplifier) {
        ...
        if (phase == 400) {
            if (entity.world.isRemote)
                entity.sendMessage(new TextComponentTranslation("gregtech.cyanide.1"));
            entity.addPotionEffect(new PotionEffect(MobEffects.NAUSEA, 1000, 9));
        } else if (phase == 300) {
            if (entity.world.isRemote)
                entity.sendMessage(new TextComponentTranslation("gregtech.cyanide.2"));
        } else if (phase == 160) {
            entity.addPotionEffect(new PotionEffect(MobEffects.BLINDNESS, 1000, 0));
        } else if (phase < 200 && phase % 5 == 0) {
            entity.attackEntityFrom(DamageSources.CYANIDE, (float) Math.pow((double) (200 - phase) / 80, 2));
            entity.hurtResistantTime = 0; // No saving you :)
        } else if (phase < 100) {
            if (entity.world.isRemote && entity.isEntityAlive())
                entity.sendMessage(new TextComponentTranslation("gregtech.cyanide.3"));
        }
    }
