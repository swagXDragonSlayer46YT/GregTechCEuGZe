...
public class GTFoodStats implements IFoodBehavior, IItemBehaviour { // These names suck
    ...
    public GTFoodStats(int foodLevel, float saturation, boolean isDrink, boolean alwaysEdible, Supplier<ItemStack> itemStackSupplier, RandomPotionEffect... potionEffects) {
    ...
    public GTFoodStats(int foodLevel, float saturation, boolean isDrink, boolean alwaysEdible, ItemStack stack, RandomPotionEffect... potionEffects) {
        this.foodLevel = foodLevel;
        this.saturation = saturation;
        this.isDrink = isDrink;
        this.alwaysEdible = alwaysEdible;
    ...
    public GTFoodStats(int foodLevel, float saturation, boolean isDrink, boolean alwaysEdible) {
        this(foodLevel, saturation, isDrink, alwaysEdible, ItemStack.EMPTY);
    }

    public GTFoodStats(int foodLevel, float saturation) {
    ...
    public GTFoodStats setReturnStack(ItemStack stack) {
        this.stackSupplier = () -> stack;
        return this;
    }
    ...
    public void addInformation(ItemStack itemStack, List<String> list) {
        if (this.potionEffects.length > 0) {
            GTUtility.addPotionTooltip(Arrays.asList(potionEffects), list);
        }
    ...
    public GTFoodStats setEatingDuration(int duration) {
        this.eatingDuration = duration;
        return this;
    }

    public GTFoodStats setPotionEffects(RandomPotionEffect... effects) {
    ...
    public GTFoodStats nutrients(float dairy, float fruit, float grain, float protein, float vegetable) {
        if (dairy > 0) {
